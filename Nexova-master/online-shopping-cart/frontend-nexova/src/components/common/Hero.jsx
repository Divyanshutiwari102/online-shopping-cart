...
function SideCard({ bg, tag, title, icon }) {
  return (
    <div className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${bg} flex flex-col justify-end p-5 min-h-[120px] lg:min-h-0 cursor-pointer group hover:-translate-y-1 transition-transform duration-200`}>
      <div className="absolute top-3 right-3 text-2xl opacity-30 group-hover:opacity-60 transition-opacity">
        {icon}
      </div>
      <span className="text-xs text-white/50 font-medium mb-1">{tag}</span>
      <span className="font-syne font-bold text-white text-base leading-snug">{title}</span>
    </div>
  )
}
