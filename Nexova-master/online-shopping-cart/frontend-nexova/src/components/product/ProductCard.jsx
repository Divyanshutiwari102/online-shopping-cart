...
export default function ProductCard({ product, onClick, onAuthRequired }) {
  ...
  const handleAddToCart = async (e) => {
    ...
    else error(result.error || 'Failed to add to cart')
    setAdding(false)
  }

  // FIX: favorite logic was inverted — liked is OLD state when this runs,
  // so we compute next state first, then use it for both setLiked and count update
  const handleFavorite = async (e) => {
    e.stopPropagation()
    if (!user) { onAuthRequired?.(); return }

    const nowLiked = !liked   // next state
    setLiked(nowLiked)
    setFavoriteCount(prev => nowLiked ? prev + 1 : prev - 1)

    try {
      if (nowLiked) await addFavorite(product.id)
    ...
    >
      <div className="relative overflow-hidden aspect-square bg-neutral-100 dark:bg-neutral-800">
        ...
        <button
          ...
          className="absolute top-2.5 right-2.5 w-8 h-8 rounded-full bg-white/90 dark:bg-neutral-800/90 backdrop-blur-sm flex items-center justify-center text-sm shadow hover:scale-110 transition-transform"
        >
          {liked ? '❤️' : '🤍'}
        </button>
