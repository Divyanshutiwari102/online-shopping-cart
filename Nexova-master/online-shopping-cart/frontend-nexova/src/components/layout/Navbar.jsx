...
export default function Navbar({ onCartOpen, onAuthOpen, onSearch, onOrdersOpen, onSellerOpen, darkMode, toggleDark }) {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white dark:bg-neutral-950 border-b border-neutral-200 dark:border-neutral-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-4">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {user ? (
            ...
            <button onClick={onAuthOpen} className="text-sm px-4 py-2 rounded-lg border border-neutral-200 dark:border-neutral-700 text-neutral-700 dark:text-neutral-200 hover:border-brand-500 hover:text-brand-500 transition-all font-medium">Sign In</button>
          )}

          <button onClick={onCartOpen} className="relative w-10 h-10 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-neutral-100 dark:bg-neutral-900 text-xl flex items-center justify-center hover:border-brand-500 transition-all">
            🛒
            {cartCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] bg-brand-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center px-1 border-2 border-white dark:border-neutral-950">
                {cartCount}
              </span>
            )}
          </button>
      ...
      <form onSubmit={handleSearch} className="sm:hidden flex px-4 pb-3 gap-2">
        <input className="flex-1 px-3 py-2 text-sm bg-neutral-100 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-lg outline-none text-neutral-800 dark:text-neutral-100 focus:border-brand-500"
          placeholder="Search..." value={q} onChange={e => setQ(e.target.value)} />
        <button type="submit" className="px-4 py-2 bg-brand-500 text-white text-sm rounded-lg font-medium">Go</button>
        <button type="button" onClick={onSellerOpen} className="px-3 py-2 bg-neutral-200 dark:bg-neutral-800 text-sm rounded-lg">🏪</button>
        {user && <button type="button" onClick={onOrdersOpen} className="px-3 py-2 bg-neutral-200 dark:bg-neutral-800 text-sm rounded-lg">📦</button>}
      </form>
    </nav>
  )
}
