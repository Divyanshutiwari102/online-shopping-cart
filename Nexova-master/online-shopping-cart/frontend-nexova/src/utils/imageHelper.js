...
export async function fetchProductImage(product, size = 'medium') {
  ...
  // Priority 3: Pexels API
  if (!PEXELS_API_KEY || PEXELS_API_KEY === 'APNI_KEY_YAHAN_DAALO') {
  ...
  try {
    ...
    const sizeMap = { tiny: photo.src.tiny, small: photo.src.small, medium: photo.src.medium, large: photo.src.large }
